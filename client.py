...
#! /usr/bin/python3
'''
Стандартные реализации алгоритма A3/A8:
COMP128
COMP128-2
COMP128-3
MILENAGE
...
def a3a8(rand, key):
    ...
    bit = [0 for x in range(128)]
    simoutput = [0 for x in range(12)]
    for i in range(16,32):
        x[i] = rand[i - 16]
    for i in range(1,9):
        for j in range(16):
            x[j] = key[j]
#  COMP128 является хеш-функцией разрядности 128 бит.
        # Разрядность аргумента 256 бит или 32 байта (128 бит Ki + 128 бит RAND).
        # 32 старших бита вычисленного значения берутся в качестве SRES, а 64 младших бита в качестве сессионного ключа Kc.
        # Пусть X [0..32] — 32байтный вход алгоритма,
        # где X [0..15] = Ki,
        # а X [16..31] = RAND.
        # T0 [0..511], T1 [0..255],T2 [0..127],T3 [0..63] и T4 [0..31] — секретные таблицы подстановки байт.
        # Алгоритм состоит из 8 раундов, в каждом раунде 5 итераций.
        # Каждая итерация заключается в поиске по соответствующей таблице (T0 для первой итерации, T1 — для второй, и т. д.)
        # и подстановке байт.
        # В конце каждого раунда, за исключением последнего, происходит перестановка полученных 128 бит результата,
        # и после перестановки эти 128 бит используется в следующем раунде.
        for j in range(5):
            for k in range(1 << j):
                for l in range(1 << (4 - j)):
                    m = l + k * (1 << (5 - j))
                    n = m + (1 << (4 - j))
                    y = (x[m] + 2 * x[n]) % (1 << (9 - j))
                    z = (2 * x[m] + x[n]) % (1 << (9 - j))
    ...
    simoutput[4 + 7] = 0
    return [simoutput[i] % 256 for i in range(0, len(simoutput))]
#преобразуем в массив.Добавляем в конец массива методом append, не забывая о позиции числа.для этого мы сначала осуществляем сдвиг вправо,для того, чтобы "отделить" добавляемое число
def to_byte_array(input):
    output = []
    for i in range(16):
        output.append((input >> i * 8) & 0xff)
    return output[::-1]
#функция запускает функцию а3а8, предварительно преобразуя полученные входные значения случ.число и ключ в массив
def run(rand, key):
    ret = a3a8(to_byte_array(rand), to_byte_array(key))
    return ret
#функция преобразования в число типа int,поскольку функция a3a8 оперирует битовыми последовательностями в виде массива
#преобразуем сдвигами влево, учитывая позицию числа
def to_int(input):
    int_val = 0
    for i in range(len(input)):
        int_val += input[i] << (len(input) - i - 1) * 8
    return int_val
